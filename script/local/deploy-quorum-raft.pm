...
my $DEPLOY_ROOT = 'deploy/quorum-raft';       # Where the files are deployed on
                                              # the workers
...
#   ...
#
# Where portA is the port used for consensus (--port paraneter in geth), port B
# is the port used for raft (--raftport in geth) and port C is the port used
# for client access (--ws.port in geth).
#
sub build_nodefile
{
    my ($path, $nodes) = @_;
...
sub dispatch
{
	...
	die ("cannot dispatch quorum-raft network to workers");
    }
}
...
sub deploy_quorum_raft
{
    ...
    #
    $proc = $RUNNER->run(\@workers, [ 'deploy-quorum-raft-worker','prepare' ]);
    if ($proc->wait() != 0) {
    ...
    if ($proc->wait() != 0) {
	die ("cannot send quorum-raft node file to worker");
    }

    $proc = $RUNNER->run(
	$genworker,
	[ 'deploy-quorum-raft-worker', 'generate', $NODEFILE_LOC ,
	  $KEYS_TXT_LOC ]
	);
    if ($proc->wait() != 0) {
	die ("failed to generate quorum-raft testnet");
    }
